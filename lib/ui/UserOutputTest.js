...
describe('highlighter', function () {
    ...
    it('highligh iterating over choosed colors', function () {
        let highlighter = formatter.createHighlighter([(text) => `red(${text})`, (text) => `blue(${text})`])

        expect(highlighter('keyword1')).to.equal('red(keyword1)')
        expect(highlighter('keyword1')).to.equal('red(keyword1)')
        expect(highlighter('keyword2')).to.equal('blue(keyword2)')
        expect(highlighter('keyword2')).to.equal('blue(keyword2)')
        expect(highlighter('keyword3')).to.equal('red(keyword3)')
    })

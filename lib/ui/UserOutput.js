...
function createSuggestionOutput(torrents, subtitles, keywords) {
    if(keywords == 0){
        return "Without keywords I can't suggest anything.\n"
    }

    try {
        var [torrent, subtitle] = core.suggestMatch(torrents, subtitles, keywords)
        ...
        return (
            ...
            + orange(`${torrent.name} / ${subtitle.subtitleName} / ${torrent.size} ${formatTorrentSeeders(torrent)}${formatTorrentLeechers(torrent)}\n`)
        )
    } catch (e) {

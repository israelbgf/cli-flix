...
function highlightMatches(torrents, subtitles, keywords, highlighter) {
    ...
    let torrentsWithHighlights = torrents
    let subtitlesWithHighlights = subtitles
    keywords.forEach((keyword) => {
        if (torrentsWithHighlights.join().includes(keyword) && subtitlesWithHighlights.join().includes(keyword)) {
            torrentsWithHighlights = torrents.map((t) => t.replace(keyword, highlighter(keyword)))
            subtitlesWithHighlights = subtitles.map((s) => s.replace(keyword, highlighter(keyword)))
        }
    })

    return [torrentsWithHighlights, subtitlesWithHighlights]
